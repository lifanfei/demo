...
<html>
  <head>
    ...
    <title>My JSP 'updateFruit.jsp' starting page</title>
	...
	function updateFruit(){
		if(blurName() && blurType() && blurCount() && blurOrigin() && blurPrice() && blurDepot()){
			document.getElementById("fruitForm").action="doUpdateFruit.action";
  ...
  <body style="background-color: #ffffcc">
       <form id="fruitForm" action="" method="post" onsubmit="return updateFruit();">
			<table id="ajaxtable" align="center">
				<tr>
					...
					<td class="textleft">
					  ...
					  <input onkeyup="value=value.replace(/[^\u4E00-\u9FA5]/g,'')" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\u4E00-\u9FA5]/g,''))" id="fruitname" onblur="blurName();" onfocus="focusName();" type="text" name="fruitname" value="${vo.fname}"/>
					  <input type="hidden" name="fruitid" value="${vo.fid}">
					</td>
					<td id="resultName"></td>
				</tr>
				<tr>
					...
					<td class="textleft">
						<input onkeyup="value=value.replace(/[^\u4E00-\u9FA5]/g,'')" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\u4E00-\u9FA5]/g,''))" id="ftype" onblur="blurType();" onfocus="focusType();" type="text" name="ftype" value="${vo.ftype}"/>
					</td>
					<td id="resultType"></td>
				</tr>
				<tr>
					...
					<td class="textleft">
						<input onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" id="count" onblur="blurCount();" onfocus="focusCount();" type="text" name="fcount" value="${vo.fcount}"/>
					</td>
					...
					<td class="textleft">
						<select id="origin" onblur="blurOrigin();" onfocus="focusOrigin();" class="selecttext" name="originid">
						  ...
						  <c:forEach var="origin" items="${olist}">
						    <option value="${origin.originid}" <c:if test="${origin.originid eq vo.originid}">selected="selected"</c:if>>${origin.originname}</option>
					...
					<td class="textleft">
						<select id="price" onblur="blurPrice();" onfocus="focusPrice();" class="selecttext" name="priceid">
						  ...
						  <c:forEach var="price" items="${plist}">
						    <option value="${price.priceid}" <c:if test="${price.priceid eq vo.priceid}">selected="selected"</c:if>>${price.price}</option>
				...
				<tr>
					...
					<select id="depot" onblur="blurDepot();" onfocus="focusDepot();" class="selecttext" name="depotid">
						  ...
						  <c:forEach var="depot" items="${dlist}">
						    <option value="${depot.depotid}" <c:if test="${depot.depotid eq vo.depotid}">selected="selected"</c:if>>${depot.depotname}</option>
				...
				<tr>
					...
					<td colspan="3" align="center">
						<input type="submit" value="修改水果信息" />
					</td>
				</tr>
